          ...
          <div className="grid min-h-[560px] overflow-hidden rounded-[30px] lg:grid-cols-[.8fr_1.2fr]">
            <div className="relative flex items-center justify-center overflow-hidden p-8 sm:p-12" style={{ background: light ? `radial-gradient(circle at 50% 45%, ${accent}16, transparent 60%), #eee9df` : `radial-gradient(circle at 50% 45%, ${accent}32, transparent 60%), #11161e` }}>
              <div className="relative h-[360px] w-[270px] sm:h-[410px] sm:w-[310px]" style={{ perspective: 1400 }}>
                {/* Recessed inner tray */}
                <motion.div className="absolute inset-0 rounded-[20px] border shadow-[inset_0_0_0_10px_rgba(0,0,0,.08),inset_0_18px_35px_rgba(0,0,0,.18),0_22px_45px_rgba(0,0,0,.25)]" style={{ background: light ? '#dcd4c7' : '#0a0e14', borderColor: light ? '#c9beae' : `${accent}55`, transformStyle: 'preserve-3d' }} initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1, transition: { duration: .55 } }}>
                  <div className="absolute inset-[13px] rounded-[15px] border p-4" style={{ background: light ? 'linear-gradient(145deg,#eee9df,#d8d0c3)' : 'linear-gradient(145deg,#171d26,#080b10)', borderColor: light ? '#c9c0b2' : 'rgba(255,255,255,.08)' }}>
                    <div className="absolute inset-[10px] rounded-[11px] border" style={{ borderColor: `${accent}33`, boxShadow: `inset 0 0 35px ${accent}12` }} />
                    <div className="relative h-full rounded-[10px] p-4">
                      <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-[.18em]" style={{ color: faint }}><span>Digital edition</span><span style={{ color: accent }}>Ready to deploy</span></div>
                      <div className="absolute left-1/2 top-1/2 w-[78%] -translate-x-1/2 -translate-y-1/2 rounded-[14px] border p-5 text-center shadow-[0_14px_28px_rgba(0,0,0,.22)]" style={{ background: light ? '#fffdf8' : '#111720', borderColor: light ? '#d8d0c2' : `${accent}55` }}>
                        <div className="mx-auto flex size-14 items-center justify-center rounded-2xl border" style={{ background: `${accent}15`, borderColor: `${accent}55`, color: accent }}><span className="text-xl font-black">{(product.name || 'A').slice(0, 1).toUpperCase()}</span></div>
                      ...
                      <div className="absolute bottom-2 left-0 right-0 flex justify-center"><span className="rounded-full border px-3 py-1 text-[8px] font-bold uppercase tracking-[.16em]" style={{ borderColor: `${accent}44`, color: accent, background: light ? '#fffdf8aa' : '#00000033' }}>Premium digital product</span></div>
                    </div>
                  </div>
                </motion.div>

                {/* Hinged front cover */}
                <motion.div className="absolute left-0 top-0 z-20 h-full w-full origin-left rounded-[18px] border-2 shadow-[18px_24px_55px_rgba(0,0,0,.35)]" style={{ background: light ? `linear-gradient(145deg, ${accent}, #8b6426 55%, #5c431f)` : `linear-gradient(145deg, ${accent}, #16191f 55%, #07090d)`, borderColor: `${accent}dd`, transformStyle: 'preserve-3d', transformOrigin: 'left center', backfaceVisibility: 'visible' }} initial={{ rotateY: 0, x: 0 }} animate={{ rotateY: -72, x: -10, transition: { duration: 1.15, delay: .35, ease: [0.16, 1, .3, 1] } }} exit={{ rotateY: 0, x: 0, transition: { duration: .65, ease: [0.7, 0, .84, 0] } }}>
