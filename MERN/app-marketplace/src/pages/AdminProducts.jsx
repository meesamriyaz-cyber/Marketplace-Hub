...
export default function AdminProducts() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-products'] }); queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] }); setEditing(null); setForm(emptyForm); setError(''); },
  ...
  const startEdit = (product) => { setEditing(product); setForm({ ...emptyForm, ...product, features: (product.features || []).join(', ') }); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const field = (key, label, type = 'text') => <label className="block"><span className="mb-2 block text-xs font-medium uppercase tracking-[.1em] text-muted-foreground">{label}</span><input type={type} value={form[key] ?? ''} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 outline-none focus:border-[#e9c878]" required={!['badge', 'install', 'art', 'accent'].includes(key)} /></label>;
