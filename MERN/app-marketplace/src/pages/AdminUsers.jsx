...
export default function AdminUsers() {
  ...
  const { data: currentUserResponse } = useQuery({ queryKey: ['current-user'], queryFn: api.auth.me });
  const currentUser = currentUserResponse?.user || currentUserResponse || null;
  ...
  return <div>
    ...
    <div className="mt-5 overflow-hidden rounded-3xl border border-border bg-surface"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-[.1em] text-muted-foreground"><tr><th className="px-5 py-4">User</th><th className="px-5 py-4">Role</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Joined</th><th className="px-5 py-4 text-right">Actions</th></tr></thead><tbody>
      {isLoading ? <tr><td colSpan="5" className="px-5 py-8 text-muted-foreground">Loading users…</td></tr> : visibleUsers.map((user) => <tr key={user._id} className="border-b border-border last:border-0"><td className="px-5 py-4"><div className="font-medium">{user.name}</div><div className="mt-0.5 text-xs text-muted-foreground">{user.email}</div></td><td className="px-5 py-4"><span className="inline-flex items-center gap-1.5">{user.role !== 'user' && <ShieldCheck className="size-3.5" />}{user.role || 'user'}</span></td><td className="px-5 py-4"><span className={user.isActive === false ? 'text-[#ee9d83]' : 'text-emerald-600'}>{user.isActive === false ? 'Inactive' : 'Active'}</span></td><td className="px-5 py-4 text-muted-foreground">{user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-IN') : '—'}</td><td className="px-5 py-4"><div className="flex justify-end gap-2"><button type="button" className="btn-ghost" onClick={() => openEdit(user)}><Edit3 className="size-4" /> Edit</button><button type="button" className="btn-ghost" onClick={() => openReset(user)}><KeyRound className="size-4" /> Reset password</button></div></td></tr>)}
    ...
    {resetting && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-[#b88445]/60 bg-[#c9a06a] text-[#2f2418] shadow-2xl dark:border-border dark:bg-surface dark:text-foreground">
        <div className="flex items-start justify-between border-b border-[#a8793d]/60 px-6 py-5 dark:border-border">
          <div><div className="text-xs font-semibold uppercase tracking-[.12em] text-[#5a4127] dark:text-muted-foreground">Account security</div><h2 className="mt-1 text-2xl font-semibold text-[#2f2418] dark:text-foreground">Reset password</h2></div>
          <button type="button" onClick={closeReset} aria-label="Close" className="rounded-xl p-2 text-[#5a4127] hover:bg-[#b88950]/40 hover:text-[#2f2418] dark:text-muted-foreground dark:hover:bg-background dark:hover:text-foreground"><X className="size-5" /></button>
        </div>
        <div className="px-6 py-5">
          <p className="text-sm leading-6 text-[#3b2b1b] dark:text-foreground">This will replace the current password for <strong className="font-semibold text-[#241a10] dark:text-foreground">{resetting.email}</strong> with a newly generated temporary password.</p>
          {targetIsAdmin && !actorIsSuperAdmin && <div className="mt-4 rounded-2xl border border-[#b95c4a]/60 bg-[#e8b09f]/55 p-4 text-sm font-medium text-[#5b2117] dark:border-[#ee9d83]/50 dark:bg-[#ee9d83]/10 dark:text-foreground">Only a Super Admin can reset an administrator password.</div>}
          {resetError && <div className="mt-4 rounded-2xl border border-[#b95c4a]/60 bg-[#e8b09f]/55 p-4 text-sm font-medium text-[#5b2117] dark:border-[#ee9d83]/50 dark:bg-[#ee9d83]/10 dark:text-foreground">{resetError}</div>}
          {temporaryPassword ? <div className="mt-5 rounded-2xl border border-[#9b6d32]/70 bg-[#e2c08a]/70 p-4 dark:border-[#e9c878]/60 dark:bg-[#e9c878]/10">
            <div className="text-xs font-bold uppercase tracking-[.1em] text-[#4b351c] dark:text-foreground">Temporary password · shown once</div>
            <div className="mt-3 flex items-center gap-2"><code className="min-w-0 flex-1 break-all rounded-xl border border-[#a8793d]/60 bg-[#f4dfb6] px-3 py-3 font-mono text-sm font-semibold text-[#2f2418] shadow-sm dark:border-border dark:bg-background dark:text-foreground">{temporaryPassword}</code><button type="button" className="rounded-xl border border-[#a8793d]/60 bg-[#f4dfb6] px-3 py-2.5 text-sm font-medium text-[#3b2b1b] hover:bg-[#f8e8c9] dark:border-border dark:bg-background dark:text-foreground dark:hover:bg-surface" onClick={copyPassword}>{copied ? <Check className="size-4" /> : <Copy className="size-4" />}{copied ? 'Copied' : 'Copy'}</button></div>
            <p className="mt-3 text-xs leading-5 text-[#4b351c] dark:text-muted-foreground">Save or securely share this password now. It will not be displayed again after closing this dialog.</p>
          </div> : <button type="button" disabled={reset.isPending || !canReset} onClick={() => reset.mutate()} className="btn-primary mt-6 w-full">{reset.isPending ? 'Generating…' : 'Generate temporary password'}</button>}
          <button type="button" onClick={closeReset} className="btn-ghost mt-3 w-full justify-center rounded-xl">{temporaryPassword ? 'Done' : 'Cancel'}</button>
        </div>
      </div>
    </div>}
  </div>;
