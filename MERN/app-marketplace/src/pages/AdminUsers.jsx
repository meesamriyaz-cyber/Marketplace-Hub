...
import { Edit3, Search, ShieldCheck, Users, X } from 'lucide-react';
...
export default function AdminUsers() {
  ...
  return <div>
    ...
    <div className="mt-7 grid gap-3 md:grid-cols-[1fr_auto_auto]">
      <div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or email…" className="input-field w-full pl-10" /></div>
      <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="input-field"><option value="all">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select>
      <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="input-field"><option value="all">All roles</option><option value="user">User</option><option value="admin">Admin</option><option value="super_admin">Super Admin</option></select>
    </div>

    <div className="mt-5 overflow-hidden rounded-3xl border border-border bg-surface"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-[.1em] text-muted-foreground"><tr><th className="px-5 py-4">User</th><th className="px-5 py-4">Role</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Joined</th><th className="px-5 py-4" /></tr></thead><tbody>
      {isLoading ? <tr><td colSpan="5" className="px-5 py-8 text-muted-foreground">Loading users…</td></tr> : visibleUsers.map((user) => <tr key={user._id} className="border-b border-border last:border-0"><td className="px-5 py-4"><div className="font-medium">{user.name}</div><div className="mt-0.5 text-xs text-muted-foreground">{user.email}</div></td><td className="px-5 py-4"><span className="inline-flex items-center gap-1.5">{user.role !== 'user' && <ShieldCheck className="size-3.5" />}{user.role}</span></td><td className="px-5 py-4"><span className={user.isActive === false ? 'text-[#ee9d83]' : 'text-emerald-600'}>{user.isActive === false ? 'Inactive' : 'Active'}</span></td><td className="px-5 py-4 text-muted-foreground">{user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-IN') : '—'}</td><td className="px-5 py-4 text-right"><button type="button" className="btn-ghost" onClick={() => openEdit(user)}><Edit3 className="size-4" /> Edit</button></td></tr>)}
    ...
    {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-3xl border border-border bg-surface p-6 shadow-2xl"><div className="flex items-center justify-between"><div><div className="eyebrow">Account administration</div><h2 className="display text-2xl">Edit user</h2></div><button type="button" onClick={closeEdit} className="btn-ghost"><X className="size-5" /></button></div>
      {isSoleAdmin && <div className="mt-5 rounded-2xl border border-[#e9c878]/30 bg-[#e9c878]/10 p-3 text-sm">You are the only active administrator. Your admin role and active status cannot be removed here.</div>}
      {saveError && <div className="mt-5 rounded-2xl border border-[#ee9d83]/30 bg-[#ee9d83]/10 p-3 text-sm">{saveError}</div>}
      <div className="mt-6 grid gap-4"><label className="block"><span className="mb-1.5 block text-xs font-medium uppercase tracking-[.1em] text-muted-foreground">Name</span><input className="input-field w-full" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label className="block"><span className="mb-1.5 block text-xs font-medium uppercase tracking-[.1em] text-muted-foreground">Email</span><input className="input-field w-full" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label className="block"><span className="mb-1.5 block text-xs font-medium uppercase tracking-[.1em] text-muted-foreground">Role</span><select className="input-field w-full" value={form.role} disabled={isSoleAdmin} onChange={(e) => setForm({ ...form, role: e.target.value })}><option value="user">User</option><option value="admin">Admin</option><option value="super_admin">Super Admin</option></select></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isActive} disabled={isSoleAdmin} onChange={(e) => setForm({ ...form, isActive: e.target.checked })} /> Active account</label></div>
      <button type="button" disabled={update.isPending} onClick={() => update.mutate()} className="btn-primary mt-6 w-full">{update.isPending ? 'Saving…' : 'Save changes'}</button>
    </div></div>}
  </div>;
}
