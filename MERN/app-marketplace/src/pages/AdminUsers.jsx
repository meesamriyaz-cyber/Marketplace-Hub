    ...
    {resetting && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-border bg-surface shadow-2xl">
        <div className="flex items-start justify-between border-b border-border px-6 py-5">
          <div><div className="eyebrow">Account security</div><h2 className="display mt-1 text-2xl">Reset password</h2></div>
          <button type="button" onClick={closeReset} aria-label="Close" className="btn-ghost rounded-xl"><X className="size-5" /></button>
        </div>
        <div className="px-6 py-5">
          <p className="text-sm leading-6 text-foreground">This will replace the current password for <strong className="font-semibold text-foreground">{resetting.email}</strong> with a newly generated temporary password.</p>
          {targetIsAdmin && !actorIsSuperAdmin && <div className="mt-4 rounded-2xl border border-[#ee9d83]/50 bg-[#ee9d83]/10 p-4 text-sm font-medium text-foreground">Only a Super Admin can reset an administrator password.</div>}
          {resetError && <div className="mt-4 rounded-2xl border border-[#ee9d83]/50 bg-[#ee9d83]/10 p-4 text-sm font-medium text-foreground">{resetError}</div>}
          {temporaryPassword ? <div className="mt-5 rounded-2xl border border-[#e9c878]/60 bg-[#e9c878]/10 p-4">
            <div className="text-xs font-bold uppercase tracking-[.1em] text-foreground">Temporary password · shown once</div>
            <div className="mt-3 flex items-center gap-2"><code className="min-w-0 flex-1 break-all rounded-xl border border-border bg-background px-3 py-3 font-mono text-sm font-semibold text-foreground shadow-sm">{temporaryPassword}</code><button type="button" className="btn-ghost shrink-0 rounded-xl" onClick={copyPassword}>{copied ? <Check className="size-4" /> : <Copy className="size-4" />}{copied ? 'Copied' : 'Copy'}</button></div>
            <p className="mt-3 text-xs leading-5 text-foreground/80">Save or securely share this password now. It will not be displayed again after closing this dialog.</p>
