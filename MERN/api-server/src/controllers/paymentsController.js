...
export const verifyRazorpayPayment = async (req, res) => {
  ...
  order.status = 'paid';
  order.payment = { ...order.payment, provider: 'razorpay', razorpayOrderId: razorpay_order_id, razorpayPaymentId: razorpay_payment_id, razorpaySignature: razorpay_signature, paidAt: new Date() };
  await order.save();

  await Promise.all((order.items || []).map((item) => Product.findByIdAndUpdate(item.productId, { $inc: { salesCount: item.quantity || 1 } })));
